...
func (as *PostRelationClient)FetchComments(fetchCommentReq requestmodels.FetchCommentsReqeust)(responsemodels.FetchCommentsResponse,error){
	resp,err:=as.Client.FetchComments(context.Background(),&post_relation.FetchCommentsRequest{
		PostId: fetchCommentReq.PostID,
	})
	//var resp *post_relation.FetchCommentsResponse
	if err!=nil{
		return responsemodels.FetchCommentsResponse{},err
	}
	comments:=make([]responsemodels.Comment,len(resp.Comments))
	for i,v:=range resp.Comments{
		comments[i].ID=v.Id
		comments[i].CommentText=v.CommentText
	}
...
func (as *PostRelationClient)FetchCommentsOfComment(fetchCommentsOfCommentReq requestmodels.FetchCommentsOfCommentReqeust)(responsemodels.FetchCommentsOfCommentResponse,error){
	resp,err:=as.Client.FetchCommentsOfComment(context.Background(),&post_relation.FetchCommentsOfCommentRequest{
		PostId: fetchCommentsOfCommentReq.PostID,
		ParentCommentId: fetchCommentsOfCommentReq.ParentCommentId,
	})
	if err!=nil{
		log.Println("error in grpc calling fetch commments of comment",err)

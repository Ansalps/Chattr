...
func StartNotificationConsumer(brokerStr string, topic string, hub *websockethub.Hub, notificationUsecase interfacesUsecase.NotificationUsecase,
	log logger.Logger, caCert, accessCert, accessKey []byte) {
	// 1. Setup TLS Configuration
	cert, err := tls.X509KeyPair(accessCert, accessKey)
	if err != nil {
		log.Fatal("failed to load client cert/key",
	...
	// 3. Setup the Reader with the Dialer
	reader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:  strings.Split(brokerStr, ","),
		Topic:    topic,
		GroupID:  "notification-group", // This ensures we don't miss messages
		Dialer:   dialer, // Crucial: Tell the reader to use TLS
		MinBytes: 10e3,                 // 10KB
		MaxBytes: 10e6,                 // 10MB
	})
